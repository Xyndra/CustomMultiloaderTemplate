package de.xyndra.examplemod.utils
...
object LoadUtil {
    ...
    fun processClass(className: String, classLoader: ClassLoader = Thread.currentThread().contextClassLoader) {
        ...
        if (hasAnnotation(className, "Lde/xyndra/${ProjectProps["modId"]}/utils/LoadKt;")) {
        ...
        } else if (hasAnnotation(className, "Lde/xyndra/${ProjectProps["modId"]}/utils/LoadJ;")) {
            val clazz = classLoader.loadClass(className)
            val loadMethod: Method = try {
